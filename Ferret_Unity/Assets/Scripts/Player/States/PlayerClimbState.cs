...
public class PlayerClimbState : IState
{
  ...
  {
      if( (m_playerManager.RayCastDownToStopClimbing() == false) && !m_endOfClimbState && !m_playerManager.IsInLerpRotation && m_playerManager.EndOfFirstClimbSnap 
      && m_playerManager.ClimbArea.m_areaCanBeFinishedClimbable){
  ...
  public void Update()
  {
    ...
    if(m_playerManager.CanMoveOnClimb && m_playerManager.m_jumpButton && !m_playerManager.IsInLerpRotation && !m_playerManager.m_playerDebugs.m_pauseGame.m_pause && m_playerManager.CanJumpAfterEndPause){
